...
class GeneticPlayer:    # forward-propagating neural network

    def __init__(self, pop_size, num_generations, num_trails, window_size, hidden_size, board_size,
                 mutation_chance, mutation_size):
        self.pop_size = pop_size  # population of brains
        self.num_generations = num_generations
        self.num_trails = num_trails  # number of games each brain is gonna play
        self.window_size = window_size  # how much board a brain can see
        self.hidden_size = hidden_size  # size of the brain
        self.board_size = board_size

        self.mutation_chance = mutation_chance  # probability that mutation is introduced when a new generation spawns
        self.mutation_size = mutation_size  # magnitude that those mutation will occur at
        ...
        self.pop = [self.generate_brain(self.window_size**2, hidden_size, len(MOVES)) for _ in range(self.pop_size)]
    ...
    def proccess_board(self, board, x1,y1, x2, y2):
        # x and y are positions of the snaake
    ...
    def one_generation(self):
        ...
        # DEBUG
        print(scores)
        top_25 = [self.pop[i] for i in top_25_indexes][::-1]        # [::-1] to reverse the list and the best brain is now in the first index
    ...
    def evolve_pop(self):
        ...
        for brain in self.pop:
            ...
            print("Snake lenght", len(game.snakes[0]))
